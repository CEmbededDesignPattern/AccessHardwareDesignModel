 ...
 * connands to the various servos to which the RobotArmManager connects. In actual practice
 ...
 *  1. zeros the servos
 *  2. computes tje trajectory with call to computeTrajectory()
 *  3. executes each step in the constucted action list
  */
...
int RobotArmManager_zero(RobotArmManager *const me)
{
    ...
    if (me->itsGraspingManipulator == NULL)
        return -1;
    if (GraspingManipulator_zero(me->itsGraspingManipulator))
        return -1;
